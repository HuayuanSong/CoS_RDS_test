...
import random
docs = '''\nThis is a Digit Span Test that evaluates working memory performance. Try to remember the digits in the order that they are presented and repeat them once the sequence has stopped. If successful, the length of the sequence will increase by 1. You can enter three incorrect sequences before the test gets terminated. We will start out with 2 digits.\nClick 'Start' or press 'Enter' if you are ready to start the test.'''
...
def startTest(*args):
    ...
    while True:
        ...
        seqtxt = canvas.create_text(Width/2,Height/3.5,fill='darkblue',font='Arial 32',text=txt,justify='c')
        ...
        for a in range(length):
            z = canvas.create_text(Width/2, Height/2, fill='darkblue',font='Times 160',text=seq_digits[a],justify='c')
            canvas.after(1000,canvas.update())
            canvas.delete(z)        
        label = canvas.create_text(Width/2,Height/2.3,fill='darkblue',font='Arial 26',text='Repeat the sequence here',justify='c')
        ...
        def get_text(event=None):
            ...
            if content == seq_digits:
                canvas.create_text(Width/2,Height/2.3,fill='darkblue',font='Arial 26',text='Correct! Continue...',justify='c')
                canvas.after(1200, delete)
            else:
                canvas.create_text(Width/2,Height/2.3,fill='darkblue',font='Arial 26',text='Try again!',justify='c')
                FAILURES += 1
                if FAILURES < 1:
                    global i, DIGITS
                    i -= 1
                    ...
                    canvas.delete('all')
                    canvas.create_text(Width/2,Height/2.3,fill='darkblue',font='Arial 26',text='Thank you for your participation!',justify='c')
                    canvas.create_text(Width/2,Height/2,fill='darkblue',font='Arial 36',text='Your score: {0}'.format(counter),justify='c')
                    wdw.after(3000, lambda: wdw.destroy())
        button2 = tk.Button(wdw,height=2,width=10,text='Continue',font='Arial 20',fg='black',command=get_text, bd=0)
...
wdw.state('zoomed')
canvas.create_text(Width/2, Height/4.5, fill='darkblue',font='Arial 52',text=title,justify='c')
canvas.create_text(Width/2, Height/2.2, fill='darkblue',font='Arial 36',text=docs,justify='c')
i=0
DIGITS=1
FAILURES=1
wdw.bind('<Return>', lambda event: startTest())
userNumbers, generatedNumbers = [], []
button1=tk.Button(wdw, height=2, width=8, text='Start',font='Arial 24',fg='black',command=startTest,bd=0)
button1.configure(bg='#4682B4',activebackground='#36648B',activeforeground='white')            
